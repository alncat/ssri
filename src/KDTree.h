...
public:
    ...
    void kNN(const Point<N>& key, std::size_t k, std::vector<std::pair<Point<N>, ElemType>>& res) const;
...
private:
    ...
    void nearestNeighborRecurse(const Node* currNode, const Point<N>& key, BoundedPQueue<std::pair<Point<N>, ElemType> >& pQueue) const;
...
template <std::size_t N, typename ElemType>
void KDTree<N, ElemType>::nearestNeighborRecurse(const typename KDTree<N, ElemType>::Node* currNode, const Point<N>& key, BoundedPQueue<std::pair<Point<N>, ElemType> >& pQueue) const {
    if (currNode == NULL) return;
    const Point<N>& currPoint = currNode->point;

    // Add the current point to the BPQ if it is closer to 'key' that some point in the BPQ
    pQueue.enqueue(std::make_pair(currNode->point, currNode->value), Distance(currPoint, key));
...
template <std::size_t N, typename ElemType>
void KDTree<N, ElemType>::kNN(const Point<N>& key, std::size_t k, std::vector<std::pair<Point<N>, ElemType>>& res) const {
    BoundedPQueue<std::pair<Point<N>, ElemType> > pQueue(k); // BPQ with maximum size k
    ...
    // Count occurrences of all ElemType in the kNN set
    while (!pQueue.empty()) {
        res.push_back(pQueue.dequeueMin());
    }
