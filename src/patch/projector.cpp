...
void Projector::initialiseData(int current_size)
{
	...
	case 3:
       ...
       variance.resize(pad_size, pad_size, pad_size / 2 + 1);
       //variance_coarse.resize(r_max*2 + 1, r_max*2 + 1, r_max + 1);
	   break;
	default:
	...
	data.setXmippOrigin();
    variance.setXmippOrigin();
    //variance_coarse.setXmippOrigin();
	data.xinit=0;
    variance.xinit = 0;
    //variance_coarse.xinit = 0;
}
...
void Projector::computeFourierTransformMap(MultidimArray<RFLOAT> &vol_in, MultidimArray<RFLOAT> &power_spectrum, int current_size, int nr_threads, bool do_gridding, bool do_heavy)
{
    ...
    if(MULTIDIM_SIZE(old_variance) < MULTIDIM_SIZE(variance))
        FOR_ALL_ELEMENTS_IN_ARRAY3D(old_variance)
	    {
	...
	if(do_heavy)
		FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY1D(power_spectrum)
		{
            ...
            power_spectrum_avg += DIRECT_A1D_ELEM(power_spectrum, i);
