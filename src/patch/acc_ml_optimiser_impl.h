		...
		)
{
	...
	for (unsigned long ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle].particles_id.size(); ipart++)
	{
        ...
        if (!baseMLO->do_zero_mask) // prepare a acc-side Random image
        {
                ...
                int seed(baseMLO->random_seed + my_ori_particle);
	...
	for (long int ipart = 0; ipart < sp.nr_particles; ipart++)
	{
		...
		for (unsigned long iclass = sp.iclass_min; iclass <= sp.iclass_max; iclass++)
		{
			...
			runWavgKernel(
					...
					&(~wdiff2s_AA)[AAXA_pos],
					&(~wdiff2s_XA)[AAXA_pos],
                    //accMLO->bundle->backprojectors[iproj].d_mdlVar,//variance,
                    //accMLO->bundle->backprojectors[iproj].d_mdlWeightNorm,//weight norm
					op,
					orientation_num,
					translation_num,
					image_size,
