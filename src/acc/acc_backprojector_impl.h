			...
			int max_r, int paddingFactor)
{
		...
		paddingFactor != padding_factor)
	{
		...
		allocaton_size = mdlXYZ * sizeof(XFLOAT) * 5;
	}
...
void AccBackprojector::getMdlData(XFLOAT *r, XFLOAT *i, XFLOAT * w, XFLOAT * var, XFLOAT * weight_norm)
{
#ifdef CUDA
	DEBUG_HANDLE_ERROR(cudaStreamSynchronize(stream)); //Make sure to wait for remaining kernel executions
...
void AccBackprojector::clear()
{
	...
	if (d_mdlReal != NULL)
	{
		...
		d_mdlVar = d_mdlWeightNorm = d_mdlReal = d_mdlImag = d_mdlWeight = NULL;
	}
}
