...
public:
	...
	CudaUnifedPtr<cufftDoubleReal> reals;
	CudaUnifedPtr<cufftDoubleComplex> fouriers;
#else
	CudaUnifedPtr<cufftReal> reals;
	CudaUnifedPtr<cufftComplex> fouriers;
#endif
	cufftHandle cufftPlanForward, cufftPlanBackward;
	int direction;
	int dimension, idist, odist, istride, ostride;
	...
	CudaFFTU(cudaStream_t stream, int gpu_id, int transformDimension = 2):
		reals(stream, gpu_id),
		fouriers(stream, gpu_id),
		cufftPlanForward(0),
		cufftPlanBackward(0),
		direction(0),
		dimension((int)transformDimension),
...
//		printf("%15li\n", avail);
    ...
    void backward(CudaUnifedPtr<cufftReal> &dst)
