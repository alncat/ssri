...
void cuda_lasso(int fsc143, int tv_iters, RFLOAT l_r, RFLOAT mu, RFLOAT tv_alpha, RFLOAT tv_beta, RFLOAT eps, MultidimArray<RFLOAT> &Fconv,
    ...
    if(do_nag) {
        ...
        fconv_norm = sqrt(fconv_norm/Fconv.nzyxdim*8);
        //for(int k = 0; k < ZZ; k++)
        //    for(int i = 0; i < YY; i++)
        //        for(int j = 0; j < XX; j++){
    ...
    std::cout << "optimization parameters: " << "implicit_weight: " << lambda <<  " max weight: " << max_weight << " min weight: " << min_weight << " condition number: " << max_weight/min_weight << std::endl;
    //tv_alpha *= std::sqrt(normalise);
    //tv_beta *= std::sqrt(normalise);
    std::cout << "yob_norm: " << fconv_norm << " avg weight: " << normalise << " original_img_norm: " << yob_norm <<" img_size " << img_size << " FBsize " << FBsize << std::endl;
    ...
    for(int eps_i = 0; eps_i < 2; eps_i++){
        ...
        for(int beta_i = 0; beta_i < 4; beta_i++){
            ...
            if(min_err > test_err) {
                ...
                for(int i = 0; i < img_size; i++){
                    best_img.data[i] = img[i]; //(1. + w/normalise)*img[i];
                }
            }
        ...
        for(int beta_i = 0; beta_i < 1; beta_i++){
            ...
            FOR_ALL_ELEMENTS_IN_FFTW_TRANSFORM(Fweight){
                if(kp*kp + ip*ip + jp*jp > 4*fsc143*fsc143) {
                    ...
                    int iw = ip;
                    int jw = jp;
                    if(kp < 0) kw += Fdata.zdim;
                    if(ip < 0) iw += Fdata.ydim;
                    int index = kw*Fdata.ydim*Fdata.xdim + iw*Fdata.xdim + jw;
                    RFLOAT w = FFTW_ELEM(Fweight, kp, ip, jp);
                    if (w < 1.) continue;
                    RFLOAT diff_x = transformer.fouriers[index].x*scale - Fdata.data[index].real/w;
                    RFLOAT diff_y = transformer.fouriers[index].y*scale - Fdata.data[index].imag/w;
                    test_err += (diff_x*diff_x + diff_y*diff_y)*w;
                    avg_F += (Fdata.data[index].real/w)*(Fdata.data[index].real/w)*w;
                    avg_F += (Fdata.data[index].imag/w)*(Fdata.data[index].imag/w)*w;
                    test_counter += 1;
                }
            }
            test_err /= test_counter;
